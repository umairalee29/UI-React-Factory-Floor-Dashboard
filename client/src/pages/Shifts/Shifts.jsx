...
export default function Shifts() {
  ...
  const { summaries, loading } = useSelector((s) => s.shifts);

  useEffect(() => {
    dispatch(fetchShifts());
  }, [dispatch]);

  // Build chart data: group by date, one entry per date with morning/afternoon/night OEE
  const byDate = {};
  summaries.forEach((s) => {
  ...
  const chartData = Object.values(byDate).slice(0, 7).reverse();

  // Latest day summary per shift
  const latestByShift = {};
  summaries.forEach((s) => {
                ...
                return (
                  <div key={shift} className={styles.shiftCard} style={{ borderTopColor: SHIFT_COLORS[shift] }}>
                    ...
                    <div className={styles.shiftMeta}>
                      ...
                      <span className={s?.faults_count > 0 ? styles.faultText : ''}>
                ...
                <tbody>
                  {summaries.map((s) => (
                    <tr key={s._id}>
                      ...
                      <td className={styles.shiftCell} style={{ color: SHIFT_COLORS[s.shift] }}>
                        {s.shift}
                      </td>
